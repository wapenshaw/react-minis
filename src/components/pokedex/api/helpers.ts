...
export function getHighResImage(id: number): string {
	if (id < POKE_COUNT) {
		let url: string;
		if (id < 10) url = `https://assets.pokemon.com/assets/cms2/img/pokedex/detail/00${id}.png`;
		else if (id < 100) url = `https://assets.pokemon.com/assets/cms2/img/pokedex/detail/0${id}.png`;
		else url = `https://assets.pokemon.com/assets/cms2/img/pokedex/detail/${id}.png`;
		return url;
	} else return '';
}
